...
def uptoBucket(local_up_dir, bucket):
    ...
    for filename in files_list:
        ...
        with open(filename, 'rb') as f:
            try:
                bucket.put(toBucketPath(local_up_dir, filename).encode(
                    'utf-8'), f, checksum=True, headers=None)
            except Exception, e:
                print e
...
def main(BUCKETNAME, USERNAME, PASSWORD, local_dir):
    ...
    decide = raw_input(
        'Do you want to upload your output directory ( Y / N ?)')

    if decide == 'Y' or decide == 'y':
        upyun_bucket = upyun.UpYun(
            BUCKETNAME, USERNAME, PASSWORD, timeout=30, endpoint=upyun.ED_AUTO)
